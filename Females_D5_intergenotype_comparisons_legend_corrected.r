...
HembrasD5 <- read_excel("HembrasD5.xlsx")     

# Generate vectors with data                                                                         

w1118 <- HembrasD5$w1118
prtpΔ1 <- HembrasD5$prtpΔ1
parkina <- HembrasD5$parkina
prtpΔ1parkina <- HembrasD5$prtpΔ1parkina
...
write.xlsx(results, "levene_results_HembrasD5.xlsx" , sheetName = "Results", rowNames = FALSE)

# Run Two-Way ANOVA test over prtpΔ1, prtpΔ1/parkina 

# Create data frames with the relevant data
df_subset1 <- data.frame(
  group = c(rep("prtpΔ1", length(prtpΔ1)),
            rep("prtpΔ1parkina", length(prtpΔ1parkina))),
  value = c(prtpΔ1, prtpΔ1parkina)
)

#Run t-test
t.test(value ~ group, data = df_subset1)

# Run two-way ANOVA test
ANOVA1 <- aov(value ~ group, data = df_subset1)
...
df_subset2 <- data.frame(
  group = c(rep("w1118", length(w1118)),
            ...
            rep("prtpΔ1", length(prtpΔ1)), 
            rep("prtpΔ1parkina", length(prtpΔ1parkina))),
  value = c(w1118, parkina, prtpΔ1, prtpΔ1parkina))

# Run Dunn's test
dunn_test <- dunn.test(df_subset2$value, df_subset2$group, method = "bonferroni")
